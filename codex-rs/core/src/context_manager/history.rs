...
impl ContextManager {
    ...
    // Returns the history prepared for sending to the model with reasoning removed.
    ...
    pub(crate) fn get_history_for_prompt(&mut self) -> Vec<ResponseItem> {
        let mut history = self.get_history_for_prompt_including_reasoning();
        history.retain(|item| !matches!(item, ResponseItem::Reasoning { .. }));
        history
    }

    // Returns the history prepared for sending to the model, including reasoning items.
    // With extra response items filtered out and GhostCommits removed.
