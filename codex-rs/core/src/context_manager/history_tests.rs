...
#[test]
fn get_history_for_prompt_excludes_reasoning_items() {
    let reasoning = reasoning_msg("thinking...");
    let user = user_msg("hi");
    let mut history = create_history_with_items(vec![reasoning, user.clone()]);

    let filtered = history.get_history_for_prompt();

    assert_eq!(filtered, vec![user]);
}

#[test]
fn get_history_for_prompt_including_reasoning_keeps_reasoning_items() {
    let reasoning = reasoning_msg("thoughts");
    let assistant = assistant_msg("result");
    let mut history = create_history_with_items(vec![reasoning.clone(), assistant.clone()]);

    let filtered = history.get_history_for_prompt_including_reasoning();

    assert_eq!(filtered, vec![reasoning, assistant]);
}
