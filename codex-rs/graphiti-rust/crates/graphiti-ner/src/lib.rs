...
impl Default for RuleBasedExtractor {
    fn default() -> Self {
        ...
        patterns.insert(EntityLabel::Organization, vec![
            ...
            Regex::new(r"\b[A-Z][A-Za-z&\s]+(Inc|Corp|Corporation|Company|Ltd|LLC|Group|Organization|Association|Institute|University|College|Hospital|Bank)\b").unwrap(),
        ]);
...
impl EntityExtractor for RuleBasedExtractor {
    ...
    async fn extract(&self, text: &str) -> Result<Vec<ExtractedEntity>> {
        ...
        debug!("Extracted {} entities", filtered.len());
        Ok(filtered)
    }
}
