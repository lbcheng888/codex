...
impl Tool for DeleteLines {
    ...
    #[instrument(skip_all, fields(path = tracing::field::Empty, start_line = tracing::field::Empty, end_line = tracing::field::Empty, lines_deleted = tracing::field::Empty))]
    async fn run(&self, input: Value, ctx: ToolContext) -> serena_core::error::Result<Value> {
        ...
        let lines_deleted = end_idx - start_idx;
        
        // Write atomically
        write_file_atomically(&abs_path, &new_content).await?;
        
        tracing::Span::current().record("lines_deleted", lines_deleted);
        tracing::info!(path = %path, start_line, end_line = actual_end_line, lines_deleted, "delete_lines completed");
